          ...
          <div class="steps-content">
           ...
           <form action="AddAlarm.php" method="post" name="user1">
              ...
              <?php 
                ...
                $UserQuery="SELECT \"Username\",\"Email\",\"organization\",\"officeno\",\"mobileno\",\"communicationway\" FROM \"tblloginandregister\"where \"approved\"='1'";
                
                $UserData=pg_query($UserQuery);


                $user_list_query="select \"Username\",\"communicationway\" from \"tblAlarmType2User\" where \"AlarmType\"='$AlarmType' and \"AlarmName\"='$AlarmName'";
              ...
              <table class="table table-bordered table-hover" style="width: 90%;margin: 0 auto;">
                <tr>
                  ...
                  <td colspan="7" style="background-color:#539CCC;color:white;text-align:center;font-weight: bold;">Group Name</td>
                  
                  <!-- <td style="background-color:#539CCC;color:white;text-align:center;font-weight: bold;">Communication Way</td> -->
                </tr>
                <?php
                  $sqlquerydata=pg_query("SELECT DISTINCT \"GroupName\" FROM \"tblloginandregister\" WHERE \"approved\"='1' and \"GroupName\"!=''");
                  $arr=array();
                  if(pg_num_rows($sqlquerydata)>0){
                    while ($row=pg_fetch_array($sqlquerydata)) {
                      $ray=explode(",", $row[0]);
                    ...
                    ?>
                      <tr>
                        <td><input type="checkbox" class="checkboxes" name="users[]" <?PHP $user_list_set= pg_query($user_list_query);while ($row=pg_fetch_array($user_list_set)) { if (trim("Group - ".$array[$k])==trim($row[0])) { echo "checked"; $com_way=$row[1];} } ?> value="<?php echo "Group - ".$array[$k]; ?>"  /></td>
                        <td colspan="6"><?php echo "Group - ".$array[$k]; ?></td>
                        
                        <!-- <td>
                          <select name='<?php echo "Group_-_".$array[$k]; ?>'>
                    ...
                    ?>
                      <tr>
                        <td colspan="7">No User Groups Available</td>
                      </tr>
                    <?php
                  }
                ?>
                <tr>
                  ...
                  <td colspan="2" style="background-color:#539CCC;color:white;text-align:center;font-weight: bold;">User Name</td>
                  <td style="background-color:#539CCC;color:white;text-align:center;font-weight: bold;">Email</td>
                  <td style="background-color:#539CCC;color:white;text-align:center;font-weight: bold;">Organization</td>
                  <td style="background-color:#539CCC;color:white;text-align:center;font-weight: bold;">Office No.</td>
                  <td style="background-color:#539CCC;color:white;text-align:center;font-weight: bold;">Mobile No.</td>
                  <td style="background-color:#539CCC;color:white;text-align:center;font-weight: bold;">Communication Way</td>
                </tr>
                <?php
                  while ($getdata=pg_fetch_array($UserData)) {
                    ...
                    ?>
                      <tr>
                        <td><input type="checkbox" class="checkboxes" name="users[]" <?PHP $user_list_set= pg_query($user_list_query);while ($row=pg_fetch_array($user_list_set)) { if (trim($getdata["Username"])==trim($row[0])) { echo "checked"; $com_way=$row[1];} } ?> value="<?php echo $getdata["Username"]; ?>"  /></td>
              ...
              <?php 
                if (trim($AlarmType)=="single") {
                  ?>
                    ...
                    <input type="hidden" value="<?php if (isset($_POST['Time'])) {echo $_POST['Time'];}?>" name="Time">
                    <input type="hidden" value="<?php if (isset($_POST['deadband'])) { echo $_POST['deadband'];}?>" name="deadband">
                  <?php 
                }
                else
                {
                  ?>
                    ...
                    <input type="hidden" value="<?php if (isset($_POST['times'])) { echo  htmlentities(serialize($_POST['times']));}?>" name="Time">
                  <?php
                }
              ?>
