...
if(isset($_POST['chekedstn']))
{
  ...
  foreach($checkbox1 as $chk1)  
   ...
   //echo "<script>window.location.href='report-old.php?select1='+$stnnames<script>";
 
  // header("Location : report-old.php?select1=$stnnames");
}
?>
...
<script src="https://cdnjs.cloudflare.com/ajax/libs/bootstrap-select/1.13.12/js/i18n/defaults-am_ET.min.js"></script>
  ...
  return;
          ...
          if (type=="Daily") {
           ...
           // var params = document.getElementById("SensorList").value;
              ...
              window.location.href= 'report-old.php?FY=' + yearFrom + "&FM=" + monthFrom + "&FD=" + dayFrom + "&PARAMS=" + params+"&station="+station+"&type=Daily&report_type="+typereport;
             }
           ...
           var yearFrom=$("#years option:selected").val();
            var monthFrom=$("#months option:selected").val();
              window.location.href= 'report-old.php?FY=' + yearFrom + "&FM=" + monthFrom + "&PARAMS=" + params+"&station="+station+"&type=Monthly&report_type="+typereport;
         }
         else if (type=="Annual")
         {

           var yearFrom=$("#Annual_years option:selected").val();
              window.location.href= 'report-old.php?FY=' + yearFrom + "&PARAMS=" + params+"&station="+station+"&type=Annual&report_type="+typereport;
         }
         }
...
$("#report_type").change(function()
  {
    var val=$("#report_type").val();
       ...
       window.location.href="report-old.php?report_type="+val;
...
$(document).ready(function()
  {
    ...
    function GetData()
    {
      ...
      window.location.href='report-old.php?station='+stn+"&report_type="+val;//"&type="+type_of_report;
...
<div class="col-md-2" > 
  ...
  if(trim($_GET['report_type'])=="Custom") echo "selected";
	...
	<div class="col-md-2" >  <input type="text" id="Sname"  placeholder="Station Name" name="StationName" class="form-control" required onchange="getSensors()" value='<?php 
...
<?php if (isset($_GET['FY'])&&isset($_GET['FM'])&&isset($_GET['FD'])) {
      ...
      $da= strtotime( $_GET['FY'].'/'.$_GET['FM'].'/'.$_GET['FD']);
    	...
    	<td>
		 ...
		 <input type="date" name="EditStartDate" id="EditStartDate" class="form-control" required value="<?php if (isset($_GET['type'])) {
          if(trim($_GET['type'])=="Daily"){
             echo date('Y-m-d',$da);} 
          else if(trim($_GET['type'])=="Monthly"){
              echo date('m-d',$da);} 
          else{
              echo date('Y',$da);} 
            }?>" onChange="emptyHours()">
...
function graph($vertualsensors,$realsensors,$v_sensor_names,$r_sensor_names)
{
  ...
  if (isset($result_table)) {
    ...
    if ($_GET['type']=="Daily") {
      ...
      $sql_queryReal ="SELECT * FROM crosstab ('SELECT  (''20''||\"Year\"::text ||''-''||\"Month\"::text ||''-''|| \"Day\" ::text ||'' ''||\"Hour\"::text || '':'' || \"Minute\" ::text || '':'' ||\"Second\")::timestamp  as t,\"HydroMetParamsTypeId\", \"Value\" FROM \"'$table'\" a where ((\"Year\" ,\"Month\", \"Day\") BETWEEN ($FY,$FM,$FD) AND ($FY,$FM,$FD))  and a.\"StationId\"=$s_id and a.\"Flag\"= ''1'' and ($HydroMetShefCodeCodition)  order by 1 desc','SELECT distinct \"HydroMetParamsTypeId\" FROM \"'$table'\" as a where ($HydroMetShefCodeCodition) and a.\"Flag\"= ''1''') AS final_result(t timestamp";
      ...
      $sql_queryVirtual ="SELECT * FROM crosstab ('SELECT  (''20''||\"Year\"::text ||''-''||\"Month\"::text ||''-''|| \"Day\" ::text ||'' ''||\"Hour\"::text || '':'' || \"Minute\" ::text || '':'' ||\"Second\")::timestamp  as t,\"HydroMetParamsTypeId\", \"VirtualValue\" FROM \"'$table'\" a where ((\"Year\" ,\"Month\", \"Day\") BETWEEN ($FY,$FM,$FD) AND ($FY,$FM,$FD))  and a.sensortype=''Virtual'' and a.\"Flag\"= ''1'' and a.\"StationId\"=$s_id and ($HydroMetShefCodeCoditionVirtual)   order by 1 desc','SELECT distinct \"HydroMetParamsTypeId\" FROM \"'$table'\" as a where a.sensortype=''Virtual'' and ($HydroMetShefCodeCoditionVirtual) and a.\"Flag\"= ''1''') AS final_result(t timestamp";
      ...
      $type="day";
      $sql_queryReal ="SELECT * FROM crosstab ('SELECT  (''20''||\"Year\"::text ||''-''||\"Month\"::text ||''-''|| \"Day\" ::text ||'' ''||\"Hour\"::text || '':'' || \"Minute\" ::text || '':'' ||\"Second\")::timestamp  as t,\"HydroMetParamsTypeId\", \"Value\" FROM \"'$table'\" a where (\"Year\"=$FY and \"Month\"=$FM)   and a.\"StationId\"=$s_id and a.\"Flag\"= ''1'' and ($HydroMetShefCodeCodition)  order by 1 desc','SELECT distinct \"HydroMetParamsTypeId\" FROM \"'$table'\" as a where ($HydroMetShefCodeCodition) and a.\"Flag\"= ''1''') AS final_result(t timestamp";
      ...
      $sql_queryVirtual ="SELECT * FROM crosstab ('SELECT  (''20''||\"Year\"::text ||''-''||\"Month\"::text ||''-''|| \"Day\" ::text ||'' ''||\"Hour\"::text || '':'' || \"Minute\" ::text || '':'' ||\"Second\")::timestamp  as t,\"HydroMetParamsTypeId\", \"VirtualValue\" FROM \"'$table'\" a where (\"Year\"=$FY and \"Month\"=$FM)  and a.sensortype=''Virtual'' and a.\"Flag\"= ''1'' and a.\"StationId\"=$s_id and ($HydroMetShefCodeCoditionVirtual)   order by 1 desc','SELECT distinct \"HydroMetParamsTypeId\" FROM \"'$table'\" as a where a.sensortype=''Virtual'' and ($HydroMetShefCodeCoditionVirtual) and a.\"Flag\"= ''1''') AS final_result(t timestamp";
    ...
    {
      $type="month";
      $sql_queryReal ="SELECT * FROM crosstab ('SELECT  (''20''||\"Year\"::text ||''-''||\"Month\"::text ||''-''|| \"Day\" ::text ||'' ''||\"Hour\"::text || '':'' || \"Minute\" ::text || '':'' ||\"Second\")::timestamp  as t,\"HydroMetParamsTypeId\", \"Value\" FROM \"'$table'\" a where (\"Year\"=$FY)   and a.\"StationId\"=$s_id and a.\"Flag\"= ''1'' and ($HydroMetShefCodeCodition)  order by 1 desc','SELECT distinct \"HydroMetParamsTypeId\" FROM \"'$table'\" as a where ($HydroMetShefCodeCodition) and a.\"Flag\"= ''1''') AS final_result(t timestamp";
      ...
      $sql_queryVirtual ="SELECT * FROM crosstab ('SELECT  (''20''||\"Year\"::text ||''-''||\"Month\"::text ||''-''|| \"Day\" ::text ||'' ''||\"Hour\"::text || '':'' || \"Minute\" ::text || '':'' ||\"Second\")::timestamp  as t,\"HydroMetParamsTypeId\", \"VirtualValue\" FROM \"'$table'\" a where (\"Year\"=$FY)  and a.sensortype=''Virtual'' and a.\"Flag\"= ''1'' and a.\"StationId\"=$s_id and ($HydroMetShefCodeCoditionVirtual)   order by 1 desc','SELECT distinct \"HydroMetParamsTypeId\" FROM \"'$table'\" as a where a.sensortype=''Virtual'' and ($HydroMetShefCodeCoditionVirtual) and a.\"Flag\"= ''1''') AS final_result(t timestamp";
    ...
    if (pg_num_rows($result_set2)>0) 
    {
      ...
      if ($_GET['report_type']=="Climate") {
        ...
        if(checkSensor($sensors_c,"Total Precipitation")){
            ...
            if($type=="Monthly")
              {
                ...
                $header_row.="<th align='center'>Arce ft.</th>";
              }
              else if($type=="Daily")
              {
                //$array_v.="<th align='center'>Discharge</th>";
        ...
        while($rows=pg_fetch_array($result_set2)) {
          ...
          if (trim($a)!="") {
            ...
            if ($_GET['report_type']=="River") {
              if($type=="Monthly")
              {
                $dis=floatval($rows[$row_count])*1.98211;
              }
              else if($type=="Daily")
              {
                $dis=floatval($rows[$row_count])*1.98211;
                ...
                $dis=floatval($rows[$row_count])*$mont*1.98211;
