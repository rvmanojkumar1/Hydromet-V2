...
$table_draw= str_replace("height: 400px", "min-height:450;max-height:500px;overflow-y:auto", $table_draw);
$table_draw= str_replace("width: 600px", "width:75%;overflow-x:auto", $table_draw);
$table_draw= str_replace("height: 200px", "min-height:280;max-height:300px;overflow-y:auto", $table_draw);
...
try
{
 //e3[i].parentElement.classList.add("mygauges");
...
interval=100;
    ...
    var minN=parseInt(maxv/1.5);
        var maxN=parseInt(maxv*1.5);

  /*  // var event= document.getElementById(data)
   //event.innerHTML="<div id='con_id"+z+"' style='width:250;height:200;'></div>";
  try
  {
  ...
  "graphset":[
    {
       ...
       "title":
       {
"text":mapping_gauge[i]
       },
       
        "scale-r":{
             "values":"0:"+maxN+":"+interval,
            "border-color":"#b3b3b3",
            "border-width":"2",
